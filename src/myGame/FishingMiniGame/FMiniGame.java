...
public class FMiniGame {
    ...
    private static FMiniGame instance;
    
    public static FMiniGame getInstance() {
        if (instance == null) {
            try {
                instance = new FMiniGame();
            } catch (IOException e) {
                e.printStackTrace();
                // Handle the error appropriately - maybe return null or throw a runtime exception
                throw new RuntimeException("Failed to initialize FMiniGame", e);
            }
        }
        return instance;
    }
    
    private FMiniGame() throws IOException {
        loadImages();
        running = false;
    ...
    public void update() throws IOException {
        if(running) {
            if (KeyHandler.getInstance().spacePressed) {
                clickCounter++;
            }
